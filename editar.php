...
if (isset($_POST['ID'])) {
    ...
    try {
        if (is_array($_FILES["files"]["name"])) {
            ...
            if (in_array($ext, $extension)) {
                if (!file_exists("../Recursos/" . $carpeta . $archivo_nombre)) {
                    ...
                    if (in_array($ext, $documentosext)) {
                        $tipoa = "Documento";
                    } else if (in_array($ext, $imagenesext)) {
                        $tipoa = "Imagen";
                    } else if (in_array($ext, $videosext)) {
                        $tipoa = "Video";
                    }elseif(in_array($ext,$audiosext)){
                        $tipoa="Audio";
                    }
        ...
        print_r($imagenes);
        $datos->modificarContenido($id, $titulo, $descripcioncorta, $descripcion, $lugar, $fecha, $imagenes, $banner, $tipo, $recurso);
        print_r("Se edito el contenido");
    } catch (Exception $ex) {
